        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              id="field-help-text"
              rows={2}
              placeholder="Необязательно — пояснение или пример, которые увидит тот, кто заполняет КЕ"
              {...register('helpText')}
            />
          </div>
          ...
          {type === 'TEXT' ? (
            <div className="flex items-center justify-between rounded-md border p-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  При заполнении КЕ введённое значение сверяется со списком
                  IP-адресов в IPAM — если адреса там нет, рядом с полем
                  покажется предупреждение. Само значение по-прежнему хранится
                  как текст, сохранить КЕ можно и с адресом, которого нет в
                  IPAM.
                </p>
              </div>
              <Controller
          ...
          {type === 'IP_REFERENCE' ? (
            <div className="rounded-md border p-3">
              <p className="text-xs text-muted-foreground">
                При заполнении КЕ нужно будет выбрать реальный адрес из IPAM
                через поиск — вписать произвольный текст нельзя. В отличие от
                «Это IP-адрес» на текстовом поле (просто подсказка), здесь
                значение — настоящая ссылка на запись в IPAM: пока адрес выбран
                хотя бы в одной КЕ, удалить его из IPAM не получится.
              </p>
            </div>
          ) : null}
          ...
          {type === 'OBJECT_REFERENCE' ? (
            <div className="space-y-3 rounded-md border p-3">
              <p className="text-xs text-muted-foreground">
                При заполнении КЕ нужно будет выбрать реальный объект через
                поиск — вписать произвольный текст нельзя. Пока объект выбран
                хотя бы в одной КЕ, удалить его не получится.
              </p>
              <div className="space-y-1.5">
              ...
              {watch('referenceTargetKind') === 'OBJECT_TYPE' ? (
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    «Любой тип объекта» — при заполнении КЕ можно будет выбрать
                    КЕ любого типа (например, для патч-кордов, где на другом
                    конце кабеля может быть сервер, коммутатор или другое
                    оборудование).
                  </p>
                </div>
              ) : null}
          ...
          {type === 'RACK_POSITION' ? (
            <div className="space-y-3 rounded-md border p-3">
              <p className="text-xs text-muted-foreground">
                При заполнении КЕ нужно будет указать начальный юнит и высоту в
                юнитах (U). Значение не проверяется на пересечение с другим
                оборудованием при сохранении — пересечения и выход за
          ...
          {type !== 'AUTO_IDENTIFIER' && type !== 'VM_IDENTIFIER' ? (
            <div className="flex items-center justify-between rounded-md border p-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Нельзя сохранить КЕ без заполнения.
                </p>
              </div>
              <Controller
                control={control}
